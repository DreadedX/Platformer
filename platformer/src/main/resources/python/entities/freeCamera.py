...
class FreeCamera(EntityInterface):
    ...
    def tick(self, entity):
        if Input.isPressed(Settings.KEY_MOD):
            self.speed = 8
        else:
            self.speed = 4

        if Input.isPressed(Settings.KEY_LEFT):
            entity.x -= self.speed

        if Input.isPressed(Settings.KEY_RIGHT):
            entity.x += self.speed

        if Input.isPressed(Settings.KEY_UP):
            entity.y -= self.speed

        if Input.isPressed(Settings.KEY_DOWN):
            entity.y += self.speed
