...
public class Text {
	...
	public static void textBox(Screen screen, String title, String msg) {
		...
		screen.drawRectangle(8, screen.height / 2 - 48, screen.width - 8,
				screen.height / 2 - 8 + 8 * height, 0xff000000);
		Font.render(title.toUpperCase(), screen, xTitle, screen.yOffset
				+ screen.height / 2 - 40);
		Font.render(msg, screen, screen.xOffset + 16, screen.yOffset
				+ screen.height / 2 - 24);
	}
	...
	public static String wrap(String msg, int maxLength, int maxLines) {
		...
		if (msg.length() > maxLength * maxLines)
			throw new RuntimeException("\"" + msg + "\" has more lines that "
					+ maxLines);
