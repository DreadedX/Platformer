...
public class Game extends Canvas implements Runnable {
	...
	public void run() {
		...
		while (running) {
			...
			if (System.currentTimeMillis() - lastTimerShort >= 100) {
				lastTimerShort += 100;
				fps = (int) ((frames)
						/ ((System.currentTimeMillis() - lastTimer) / 1000d) + 0.5d);
			}

			if (System.currentTimeMillis() - lastTimer >= 1000) {
				lastTimer += 1000;
				System.out.println(frames + " Frames, " + ticks + " Ticks, "
						+ level.particles.size() + " Particles");
				frames = 0;
				ticks = 0;
			}
		}
	...
	public void render() {
		...
		/* Debug text */
		if (debug) {
			Font.render("fps: " + fps, screen, screen.xOffset + 1,
					screen.yOffset + 1);
			Font.render("x: " + level.entities.get(0).x + " y: "
					+ level.entities.get(0).y, screen, screen.xOffset + 1,
					screen.yOffset + 9);
		}
		...
		if (input.message.isPressed()) {
			Text.textBox(
					screen,
					"Mission:",
					"It is your mission to just mess around a bit in this world and try to debug the game!");
		}
